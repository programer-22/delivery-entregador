window.onload = async function () {
  ...
  if (url === "gestordepedidos.ifood.com.br") {
    ...
    imageButton.onclick = () => {
      ...
      const orderCards = document.querySelectorAll('div[data-test-id$="-order-card"]');

      orderCards.forEach(card => {
        card.addEventListener('click', function(event) {
          console.log("Clicou em : ", event.currentTarget.innerText);
          // Remove o ícone do WhatsApp existente, se houver
          const existingWhatsAppLinks = document.querySelectorAll('.whatsapp-link');
          ...
          if (orderDetails) {
            ...
            const googleMapsLink = `https://www.google.com/maps/place/${encodeURIComponent(clientAddress.replace("●", " - "))}`;
            const wazeLink = `https://waze.com/ul?q=${encodeURIComponent(clientAddress.replace("●", " - "))}`;
            const message = `
Pedido: *${pedido}*
Cliente: *${clientName}*
*Endereço:* ${address}
...
${numberLocalizador.replace(" ", "")}
            ...
            orderDetails.querySelector(" header > div:nth-child(1) > div > div").appendChild(whatsAppLink);
          }
        }, 2000);
        });
      });
    ...
    for(let mutation of mutationsList) {
      if (mutation.type === 'childList') {
        mutation.addedNodes.forEach(node => {
          if (node.matches && node.matches('div[data-cy="modal-order-details"]')) {
            ...
            alertDiv.innerHTML = '<h3>O número para contato não pode ser encontrado automaticamente. Por favor, insira manualmente.</h3>';
            alertDiv.style = 'background-color: #f44336; color: white; position: fixed; top: 0; left: 0; width: 100%; text-align: center; padding: 10px; z-index: 9999;';
            document.body.appendChild(alertDiv);
            setTimeout(() => {
              alertDiv.remove();
            }, 3000);
            const clientName = orderDetails.querySelectorAll('div[class^="_usuarioName_"]')[0].innerText;
            const clientAddress = orderDetails.querySelectorAll('div[class^="_endereco_"]')[0].innerText;
            const clientComplement = orderDetails.querySelectorAll('div[class^="_enderecoDown_"]')[0].innerText;
            const clientAddressAdditional = orderDetails.querySelectorAll('div[class^="_enderecoReferencia_"]')[0].innerText;
            const address = clientAddress + " - " + clientComplement + " - " + clientAddressAdditional;
            const spans = orderDetails.querySelectorAll("span");
            const pedido = orderDetails.querySelector("div.name-icon-modal strong").innerText;
            const googleMapsLink = `https://www.google.com/maps/place/${encodeURIComponent(clientAddress + " , " + clientComplement)}`;
            const wazeLink = `https://waze.com/ul?q=${encodeURIComponent(clientAddress + " , " + clientComplement)}`;
            const message = `
Pedido: *${pedido}*
Cliente: *${clientName}*
*Endereço:* ${address}
...
------------------------------
            ...
            const header = orderDetails.querySelectorAll('div[class^="_headerPedido_"]')[0];
            const whatsAppLink = document.createElement("a");
            whatsAppLink.href = "https://api.whatsapp.com/send/?text=" + encodeURIComponent(message);
            whatsAppLink.target = "_blank";
            whatsAppLink.classList.add("whatsapp-link");
